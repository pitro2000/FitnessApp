...
namespace FitnessApp
{
    public partial class Schedule : Form
    {
        ...
        public Schedule()
        {
            ...
            Data_Container.protein_g = 2.2 * Data_Container.protein_;
            Data_Container.protein_kcal = 4 * Data_Container.protein_g;
            textBox_result_schedule.Text = Data_Container.result_.ToString();
            textBox_bialko.Text = Math.Round(Data_Container.protein_g, 2).ToString();
            Data_Container.fat_count_kcal = (0.2 * Data_Container.result_);
            Data_Container.fat_count = Data_Container.fat_count_kcal / 9.0;
            textBox_tluszcze.Text = Math.Round(Data_Container.fat_count, 2).ToString();
            Data_Container.carbo = (Data_Container.result_ - (Data_Container.protein_kcal + Data_Container.fat_count_kcal)) / 4.0;
            textBox_weglowodany.Text = Math.Round(Data_Container.carbo, 2).ToString();
        }
        ...
        public void Change_window()
        {
            ...
            if (window_flag == 1)
            {
                var Food_Page = new Food();

                if (!Panel_body.Contains(Food_Page.Instance))
                {
                    Panel_body.Controls.Add(Food_Page.Instance);
        ...
        private void Button_move_source_Click(object sender, EventArgs e)
        {
            window_flag = 1;
            Change_window();
        }
