...
{
    public partial class Select_FBW: Form
    {
        ...
        public void Change_window()
        {
            ...
            if (window_flag == 1)
            {
                var plan_for_b = new Plan_for_beginner();

                if (!Panel_body.Contains(plan_for_b.Instance))
                {
                    Panel_body.Controls.Add(plan_for_b.Instance);
                    plan_for_b.Instance.Dock = DockStyle.Fill;
                    plan_for_b.Instance.BringToFront();
                }
                else
                {
                    plan_for_b.BringToFront();
                }
            }
            else if(window_flag == 2)
            {
                var plan_for_a = new Advanced_Plan();

                if (!Panel_body.Contains(plan_for_a.Instance))
                {
                    Panel_body.Controls.Add(plan_for_a.Instance);
                    plan_for_a.Instance.Dock = DockStyle.Fill;
                    plan_for_a.Instance.BringToFront();
                }
                else
                {
                    plan_for_a.BringToFront();
                }
            }
        }
